...
function hasIntrinsicConstructor(prototype, name) {
	...
	try {
		return constructor.name === name && constructor.prototype === prototype && intrinsicReflectApply(intrinsicFunctionToString, constructor, []).replace(/\s+/g, " ").trim() === `function ${name}() { [native code] }`;
	} catch {
		return false;
	}
}
