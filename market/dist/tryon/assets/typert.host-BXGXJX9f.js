import{j as t,o,n as r,s as e,x as n,u as a}from"./schemas-BjdgLEE2.js";import"./json-schema-processors-6dmQcDt8.js";const s=n(e(),a()),i=e(),d=t(o({mention:e(),sessionId:n(e(),a()),label:e(),cwd:e().optional(),createdAt:r()})),m={package:"@deepseek-ai/dsh-session-reference",face:"host",schemas:[],invocations:[{id:"@deepseek-ai/dsh-session-reference#sessionReferenceResolver/candidates",service:"sessionReferenceResolver",namespace:"sessionReferenceResolver",method:"candidates",implementation:"remoteExportCandidates",invocation:{kind:"direct"},scope:{context:"agent",wire:"agentId"},parameters:[{name:"agent",wire:"agentId",source:"lookup",lookup:"agent",codec:{mode:"strict",typeSymbol:"@deepseek-ai/dsh-session/types#SessionId",schema:s}},{name:"query",wire:"query",source:"json",codec:{mode:"strict",typeSymbol:"@deepseek-ai/dsh-session-reference#sessionReferenceResolver/candidates:query",schema:i}}],cancellation:{parameter:"signal"},result:{mode:"strict",typeSymbol:"@deepseek-ai/dsh-session-reference#sessionReferenceResolver/candidates:result",schema:d},sourceLocation:{file:"packages/context/session-reference/src/index.ts",line:218,column:9}}],model:{services:[{description:"Exact-read consumer that prepares immutable cross-session message context.",summary:"Exact-read consumer that prepares immutable cross-session message context.",tags:[],jsDoc:"/** Exact-read consumer that prepares immutable cross-session message context. */",key:"sessionReferenceResolver",exportName:"SessionReferenceResolver",members:[{kind:"method",name:"listCandidates",signature:"async listCandidates( agent: Agent, query: string = '', limit: number = this.config.candidateLimit, signal?: AbortSignal, ): Promise<SessionReferenceCandidate[]>",summary:"List reference candidates, ranked by working-directory affinity.",jsDoc:`/**
 ...
 */`},{kind:"method",name:"remoteExportCandidates",signature:"@Remote('candidates') async remoteExportCandidates( agent: Agent, query: string, signal: AbortSignal, ): Promise<SessionReferenceMentionCandidate[]>",summary:"Remote face of {@link listCandidates}: the configured candidate limit applies, and every candidate carries the canonical mention a host inserts into the prompt draft.",jsDoc:`/**
 ...
 */`},{kind:"method",name:"prepare",signature:"async prepare( agent: Agent, content: ContentBlock[], references: SessionReferenceInput[], signal?: AbortSignal, ): Promise<PreparedReferencedMessage>",summary:"Snapshot all references for one accepted direct message and return one aggregated durable context.",jsDoc:`/**
 ...
 */`}],types:[{name:"Agent",declaration:`export interface Agent {
...
}`},{name:"SessionId",declaration:"export type SessionId = Branded<'SessionId'>;"},{name:"SessionReferenceCandidate",declaration:`export interface SessionReferenceCandidate {
    sessionId: SessionId;
    label: string;
    cwd?: string;
    createdAt: number;
    ...
    mention: string;
}`},{name:"SessionReferenceSource",declaration:`export interface SessionReferenceSource {
    ...
    readonly role: 'user';
}`}]}],events:[],objects:[]}};export{m as TYPERT};
