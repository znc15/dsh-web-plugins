...
# 官方实验窗口：21:35 之后（run-ab2 启动）
window_start = time.mktime(time.strptime('2026-08-15 21:35:00', '%Y-%m-%d %H:%M:%S'))
...
for mtime, d, p in rows:
    ...
    groups[grp].append({'d': d[:8], 'label': label, 'we': we, 'lm': lm, 'len': len(first), 'sample': first[:80]})
...
for grp in ['baseline','cap']:
    ...
    for r in g:
        print(f'    {r["d"]} {r["label"]:13s} we={r["we"]:2d} lm={r["lm"]:2d} len={r["len"]:4d} | {r["sample"]}')
